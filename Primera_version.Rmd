...
grafico1 %>% 
  ggplot() + 
  geom_col(aes(x = Edad, y = porcentaje_sedentarios, fill = Sexo), position = "dodge", color = "black") + geom_line(data = grafico1_H, aes(x = Edad, y = porcentaje_audifonos, colour = factor(Sexo)), linewidth = 2, color = "gold", group = 1) +
  geom_line(data = grafico1_M, aes(x = Edad, y = porcentaje_audifonos), linewidth = 2, color = "black", group = 1)+ scale_y_continuous(sec.axis = sec_axis(~., name = "Porcentaje Uso audífonos (%)")) +
  guides(x = guide_axis(angle =45)) + xlab("Grupos de edad") + ylab("Porcentaje sedentarismo (%)") + theme_dark()
