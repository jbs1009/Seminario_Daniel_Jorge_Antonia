...
```
```{r}
audif_SexAge <- audif_sexo_edad_estud %>% 
  enter_object(Data) %>% 
  gather_array %>% 
  spread_all()
audif_SexAge$Sexo <- NA
audif_SexAge$Edad <- NA
audif_SexAge$Estudios <- NA
...
for (i in 1:dim(audif_SexAge)[1]){
  ...
  audif_SexAge$Estudios[i] <- substring(audif_SexAge$Nombre[i], segunda_coma+1, tercera_coma-1)
  audif_SexAge$Audifono[i] <- substring(audif_SexAge$Nombre[i], tercera_coma+1)
}
```
