...
for (i in 1:dim(a)[1]){
  r <- unlist(gregexpr(',', a$Nombre[i]))
  primera_coma <- r[1]
  segunda_coma <- r[2]
  a$Sexo[i] <- substring(a$Nombre[i], 0, primera_coma-1)
  a$Edad[i] <- substring(a$Nombre[i], primera_coma+1, segunda_coma-1)
  a$IMC[i] <- substring(a$Nombre[i], segunda_coma+1)
}
